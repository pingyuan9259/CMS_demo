<template>
  <div class="operateExeReplyContainer">
    <!-- 顶部搜索 -->
    <el-input @keyup.enter.native="handleFilter" style="width: 150px;" class="filter-item" placeholder="学生作业内容" v-model="listQuery.content">
    </el-input>
    ...
    <el-table style="margin-top: 20px;" :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row>
      ...
      <el-table-column min-width="200px" label="学生作业内容">
        <template scope="scope">
         <!-- class="link-type" @click="goToReplyDetail(scope.row)" -->
          <span>{{scope.row.content}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="120px" label="学生姓名">
        <template scope="scope">
          <span>{{scope.row.fullname}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="学生头像" width="150px">
        <template scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      queryURL () {
        let url = '/admin/exercise/reply-top?page=' + this.currentPage + '&limit=' + this.listQuery.limit
      ...
      async getTopReplys () {
        ...
        try {
          let result = await api.getExerciseReplys({url: this.queryURL()})
      ...
      delReplyAlert (index, row) {
        if (row && row.replyId) {
          this.$confirm('将删除这篇学生作业, 是否继续?', '提示', {
