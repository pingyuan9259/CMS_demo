<template>
  <Box class="course-box">
    <span slot="box-header">编辑 推荐课程</span>
    <div slot="box-body">
      <!-- 频道列表 -->
      ...
      <div class="list-content">
        ...
        <el-button type="primary" class="course-submit"  style="height: 40px;margin-right: 20px;margin-top: 10px;" @click.prevent="editRecommendCourse">提交</el-button>
      </div>
    ...
    methods: {
      ...
      async getCourseList() {
        try {
          ...
          if (this.listQuery.name && this.listQuery.name.length) {
            url = url + '&name=' + this.listQuery.name
          }
          this.listLoading = false
          let result = await courseApi.getCourseList({url: url})
      ...
      async editRecommendCourse () {
        try {
          ...
          let result = await api.editRecommendCourse(this.reqOptions)
          this.logerrSuccess()
