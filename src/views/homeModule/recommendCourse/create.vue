<template>
  <Box class="course-box">
    <span slot="box-header">添加 推荐课程</span>
    <div slot="box-body">
      <!-- 频道列表 -->
      <div class="app-container calendar-list-container">
        <div class="filter-container">
          ...
          <el-table style="width: 100%" :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row>
            <el-table-column min-width="150px" label="课程名">
      ...
      <div class="list-content">
        ...
        <el-table style="width: 100%" :data="recommendList" v-loading.body="listLoading" border fit highlight-current-row>
          <el-table-column min-width="150px" label="推荐课程名">
            <template scope="scope">
              <span>{{scope.row.name}}</span>
            </template>
          </el-table-column>
         ...
         <el-button type="success" class="course-sort"  style="height: 40px;margin-right: 20px;margin-top: 10px;" @click.prevent="sortRecommendCourse">排序</el-button>

        <el-button type="primary" class="course-submit"  style="height: 40px;margin-right: 20px;margin-top: 10px;" @click.prevent="createRecommendCourse">提交</el-button>
      </div>
  ...
  export default {
    ...
    watch: {
      sortKey: function (val) {
        ...
        this.listQuery.is_public = ''
        switch (val) {
          case 1:
            this.listQuery.is_recommend = 1
          break
          case 2:
            this.listQuery.is_recommend = 0
          break
          case 3:
            this.listQuery.is_public = 1
          break
          case 4:
            this.listQuery.is_public = 0
          break
          default:
            this.listQuery.is_recommend = ''
            this.listQuery.is_public = ''
    ...
    methods: {
      ...
      async getCourseList() {
        try {
          ...
          let result = await courseApi.getCourseList({url: '/admin/course/course-list?offset=' + (this.currentPage - 1)*this.listQuery.limit + '&limit=' + this.listQuery.limit})
      ...
      async createRecommendCourse () {
        try {
          ...
          let result = await api.createRecommendCourse(this.reqOptions)
          this.logerrSuccess()
...
<style>
  ...
  .course-box .course-sort {
    position: fixed;
    bottom: 70px;
    right: 166px;
  }
  .course-box .course-submit {
    position: fixed;
    bottom: 70px;
    right: 66px;
  }
  ...
  .course-box .calendar-list-container {
    width: 450px;
    display: inline-block;
  }
 /* .course-box .pagination-container {
    display: inline-block;
    width: 50%;
  }*/
</style>
